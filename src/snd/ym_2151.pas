...
procedure YM2151_Init(num:byte;clock:dword;port_func:porthandler;irq_func:IRQ_Handler);
...
begin
  ...
  if sound_status.stereo then begin
    inc(audio);
    tsample[FM2151[num].tsample,sound_status.posicion_sonido]:=audio^;
    inc(audio);
    tsample[FM2151[num].tsample,sound_status.posicion_sonido+1]:=audio^;
  end else tsample[FM2151[num].tsample,sound_status.posicion_sonido]:=audio^;
end;
