...
if not(self.halt) then begin
  ...
  case instruccion of
        ...
        $07:begin   {rlca >4t<}
               r.f.c:=(r.a and $80)<>0;
               if r.f.c then r.a:=((r.a shl 1) or 1) else r.a:=r.a shl 1;
        ...
        $0f:begin   {rrca >4t<}
                r.f.c:=(r.a and 1)<>0;
                if r.f.c then r.a:=((r.a shr 1) or $80) else r.a:=r.a shr 1;
                r.f.bit5:=(r.a and $20) <>0;
                r.f.bit3:=(r.a and 8)<>0;
                r.f.h:=false;
                r.f.n:=false;
        ...
        $12:spec_putbyte(r.de.w,r.a);  {ld (DE),A >7t<}
        $13:begin {inc DE >6t<}
                self.retraso((r.i shl 8)+r.r);inc(self.contador);
                self.retraso((r.i shl 8)+r.r);inc(self.contador);
                inc(r.de.w);
...
case instruccion of
        ...
        $52:begin
              ...
              self.retraso((r.i shl 8)+r.r);inc(self.contador); 
              self.retraso((r.i shl 8)+r.r);inc(self.contador); 
              sbc_16(self.r,@r.hl,r.de.w); {sbc HL,DE >15t<}
            end;
        ...
        $a0:begin   {ldi >16t<}
                 ...
                 self.retraso(r.de.w);inc(self.contador);
                 self.retraso(r.de.w);inc(self.contador);
                 inc(r.de.w);
                 inc(r.hl.w);
                 r.f.p_v:=(r.bc.w<>0);
        ...
        $b3:begin //otir añadido el dia 18-09-04 >16t o 21t<
                ...
                if r.bc.h<>0 then begin
                  self.retraso(r.bc.w);inc(self.contador);
                  self.retraso(r.bc.w);inc(self.contador);
                  self.retraso(r.bc.w);inc(self.contador);
                  self.retraso(r.bc.w);inc(self.contador);
                  self.retraso(r.bc.w);inc(self.contador);
